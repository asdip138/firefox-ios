...
class TestSQLiteBookmarks: XCTestCase {
    ...
    func testTreeBuilding() {
        func assertTreeIsEmpty(treeMaybe: Maybe<BookmarkTree>) {
        ...
        func assertTreeContainsOnlyRoots(treeMaybe: Maybe<BookmarkTree>) {
            guard let tree = treeMaybe.successValue else {
                XCTFail("Couldn't get tree!")
                return
            }
            ...
            if case let .Folder(guid, _, children) = tree.roots[0] {
                XCTAssertEqual(guid, "root________")
                XCTAssertEqual(4, children.count)
                children.forEach { child in
                    guard case let .Folder(_, _, lower) = child where lower.isEmpty else {
        ...
        let bookmarks = SQLiteBookmarks(db: db)
        assertTreeIsEmpty(bookmarks.treeForMirror().value)
        assertTreeIsEmpty(bookmarks.treeForBuffer().value)
        assertTreeContainsOnlyRoots(bookmarks.treeForLocal().value)

        self.createStockMirrorTree(db)
        assertTreeIsEmpty(bookmarks.treeForBuffer().value)

        // Local was emptied when we moved the roots to the mirror.
        assertTreeIsEmpty(bookmarks.treeForLocal().value)
        ...
        // There's one root.
        XCTAssertEqual(1, tree.roots.count)
        if case let .Folder(guid, overridden, children) = tree.roots[0] {
        ...
        //     folderBBBBBB
        //       bookmark3001
        if case let .Folder(guidR, _, rootChildren) = tree.roots[0] {
            XCTAssertEqual(guidR, "root________")
            if case let .Folder(guidM, _, menuChildren) = rootChildren[0] {
                XCTAssertEqual(guidM, "menu________")
                if case let .Folder(guidB, _, bbbChildren) = menuChildren[0] {
                    XCTAssertEqual(guidB, "folderBBBBBB")
                    // BBB contains bookmark3001.
                    if case let .NonFolder(guidBM, _) = bbbChildren[0] {
                    ...
                    if case let .Folder(guidBF, _, _) = bbbChildren[1] {
        ...
        XCTAssertEqual(1, local.roots.count)
        if case let .Folder(guid, overridden, children) = local.roots[0] {
